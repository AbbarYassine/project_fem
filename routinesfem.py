...
def elemental_mass_matrix(node1,node2,node3):
...
def assem_mass(nodes,elements,k,interior):
    """assembly sparse mass matrix"""
    rows = []
    cols = []
    vals = []
    for p in range(1,len(elements)+1):
        if elements[p][1][0] == interior:
            Me=elemental_mass_matrix(nodes[elements[p][2][0]],nodes[elements[p][2][1]],nodes[elements[p][2][2]])   
            for row in range(3):
...
def assem_stiffness(nodes,elements,signe,interior):
    """assembly sparse stiffness matrix  """
    rows = []
    cols = []
    vals = []
    for p in range(1,len(elements)+1):
...
def assem_mass_bord(nodes,elements,cst,bord_out_tag):
    ...
    vals = []
    for p in range(1,len(elements)+1):
        if elements[p][1][0] == bord_out_tag:
            Me_bord = boundary_elemental_mass_matrix(nodes[elements[p][2][0]],nodes[elements[p][2][1]])    
            for row in range(2):
                glob_row = elements[p][2][row]
                for col in range(2):
                    glob_col = elements[p][2][col]
                    rows.append(glob_row-1)
                    cols.append(glob_col-1)
                    vals.append(cst*Me_bord[row,col])
...
def boundary_dirichlet_A(elements,D,M,M_bord,bord_in_tag):
    A = lil_matrix(M+D+M_bord)
    for p in range(1,len(elements)):
        if elements[p][1][0] == bord_in_tag:
            A[elements[p][2][0],:] = 0
            A[elements[p][2][0],elements[p][2][0]] = 1
            A[elements[p][2][1],:] = 0
            A[elements[p][2][1],elements[p][2][1]] = 1
    return csr_matrix(A)                    
